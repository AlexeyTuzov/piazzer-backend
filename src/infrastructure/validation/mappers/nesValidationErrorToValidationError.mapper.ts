...
export class NestValidationErrorToValidationErrorMapper {
  ...
  mapNestValidationErrorWithChildren(error: NestValidationError, parentPath?: string) {
    const exceptions = [];
  ...
  getValidationExceptionPath(error: NestValidationError, parentPath?: string): string {
